...
if (isset($_GET['login']) && $_GET['login'] == 'failed') {
    ?>
    <div id="login-failed">
        <p><?php echo __('Login failed: You have entered an incorrect Username or password, please try again.', 'myMovies'); ?></p>
    </div>
<?php } 
    ...
    if (isset($_GET['do']) && $_GET['do'] == 'register' || isset($_GET['action']) && $_GET['action'] == 'register') {
        ...
        if ($register) {
            ...
            <div id="login-form">
                <form role="form" action="<?php echo add_query_arg('do', 'register', home_url('/login')); ?>" method="post">
                    <h1><?php echo __('Register', 'myMovies'); ?></h1>
                    <div class="form-group">
                        <label class="sr-only" for="user"><?php echo __('Username', 'myMovies'); ?></label>
                    ...
                    <div class="form-group">
                        <label class="sr-only" for="email"><?php echo __('E-Mail','myMovies'); ?></label>
                        <input placeholder="E-Mail" class="form-control" type="text" name="email" id="user_email" />
                    </div>
            <?php do_action('register_form'); ?>
                    <hr />
                    <p><?php echo __('A password will be e-mailed to you.','myMovies'); ?></p>
                    <input class="btn btn-default" id="register" type="submit" name="submit" value="<?php echo __('Register','myMovies'); ?>" class="button" />
        ...
        <div id="login-form">
            <form role="form" action="<?php echo wp_login_url(get_permalink()); ?>" method="post">
                <h1>Login</h1>
                <div class="form-group">
                    <label class="sr-only" for="log"><?php echo __('Username', 'myMovies'); ?></label>
                ...
                <div class="form-group">
                    ...
                    <input placeholder="<?php echo __('Password', 'myMovies'); ?>" class="form-control" type="password" name="pwd" id="pwd" />
                </div>
                <div class="form-group">
                ...
                <a href="<?php echo site_url() . '/login?action=register'; ?>"><?php echo __('Register', 'myMovies'); ?></a>
                <input class="btn btn-default" type="submit" name="submit" value="<?php echo __('Login', 'myMovies'); ?>" class="button" />
